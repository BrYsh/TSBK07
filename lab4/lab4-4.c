...
GLfloat height_controll(GLfloat xf, GLfloat zf,TextureData* tex,Model* M){
    ...
    VertexIndex.y = M->indexArray[(x + z * (tex->width-1))*6 + 2];
    VertexIndex.z = M->indexArray[(x + z * (tex->width-1))*6 + 1];
    VertexIndex.w = M->indexArray[(x + z * (tex->width-1))*6 + 5];
    
    
    y11 = M->vertexArray[(x + z * tex->width)*3 + 1]; //M->vertexArray[((int)VertexIndex.x)*3 + 1];
    y21 = M->vertexArray[((x+1) + z * tex->width)*3 + 1]; //M->vertexArray[((int)VertexIndex.y)*3 + 1];
    y12 = M->vertexArray[(x + (z+1) * tex->width)*3 + 1]; //M->vertexArray[((int)VertexIndex.z)*3 + 1];
